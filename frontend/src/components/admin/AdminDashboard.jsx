...
import { calculateExpiration, calculateDistance, calculateDateDifferenceInMinutes } from '../../commonly used functions/functions'
...
function AdminDashboard() {
  ...
  const currentRows = places.slice(indexOfFirstRow, indexOfLastRow);
  const totalRows = places.length;
          ...
          <div class=" min-w-full inline-block align-middle">
            <div className="flex flex-row justify-between items-center px-2 py-3">
              <div className="flex flex-col items-start gap-1 justify-center">
                <h1 className=' text-lg font-semibold text-[#101828]'>Current Locations</h1>
                  ...
                  <tbody class="divide-y divide-gray-200 ">
                    {places.map((row, index) => (
                      <tr key={index}>
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm font-medium text-[#101828] ">{row.name}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-[#4E7690] font-normal">{row.address}</td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-[#4E7690] font-semibold ">{calculateDistance(row.coordinates[0], row.coordinates[1], center)}</td>
                        <td class={`px-4 py-3 whitespace-nowrap text-sm  text-gray-800 `}>
                          <h1 className={`w-fit flex flex-row gap-1 justify-center items-center rounded-[50px] py-1 px-3 ${row.expiration === -1 ? "text-[#037847]  bg-[#ECFDF3]" : "text-[#364254] bg-[#F2F4F7]"}`}>
                            <GoDotFill className='  rounded-full' />
                            {row.expiration==-1?"Permanent":"Temporary"}
                          </h1>
                        </td>
                        <td class="px-6 py-4 whitespace-nowrap text-sm text-[#4E7690] font-semibold ">{((row.expiration-calculateDateDifferenceInMinutes(Date().toString(),row.createdAt))/60).toFixed(2)}</td>
