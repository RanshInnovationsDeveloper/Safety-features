  ...
  async (req, res) => {
    try {
      ...
      if (user) {
        return res
          .status(400)
          .json({ success: false, error: "This userid is already taken" });
      }
    ...
    try {
      ...
      if (!user) {
        ...
        return res
          ...
          .json({
            success,
            errror: "Invalid username enter valid credentials",
          });
      }
      ...
      if (!passwordCompare) {
        ...
        return res
          .status(400)
          .json({ success, errror: "password Enter valid credentials" });
      }

      const data = {
        // here we have make a const named data in which we have stored computergeneterated id of user so we can make token for the user. you will be able to see tthis id when you are going to run this request on thunder client
        user: {
          id: user.id,
        },
      };
